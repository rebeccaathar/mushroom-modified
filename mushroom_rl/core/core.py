...
class Core(object):
    ...
    def __init__(self, agent, mdp, callbacks_fit=None, callback_step=None,
        ...
        #I had to bring the MDPinfo Class and change it here
        #---------------MDP Propeties-------------------------------
        from mushroom_rl.core import  MDPInfo
   
        observation_space = self.mdp.observation_space
        action_space = self.mdp.action_space
        horizon = 100
        gamma = 0.9
        self.mdp_info = MDPInfo(observation_space, action_space, gamma, horizon)
        #------------------------------------------------------------

        #I had to change the initial state to adapt the code to Bsenv
        current_state_index = randint(0, self.mdp.get_num_states())
        ...
        self.initial_states = current_state_index
        self._reward_acc = []
        self.list_reward =[]
        self._acc_steps_eps = []
    ...
    def _run_impl(self, move_condition, fit_condition, steps_progress_bar,
        ...
        #showing the accumulated reward per episode 
        print(self.acc_reward_per_episode())
    ...
    def _step(self, render):
        ...
        next_state, reward, absorbing, _ = self.mdp.step(action_index)

        self._episode_steps =+ 1   
        
        ''' vai guardar todos os rewards daquele episódio'''

        self._reward_acc.append(reward)
                
        if render:
            self.mdp.render()

        last = not(
            self._episode_steps < self.mdp_info.horizon and not absorbing)

        state = self.initial_states
       
        next_state = self._preprocess(copy.copy(next_state))
        self._state = next_state

        return state, action_index, reward, next_state, absorbing, last

# ------------- Accumulated Reward per Episode -------------------- 

    def acc_reward_per_episode(self):

        import matplotlib.pyplot as plt
    
        total_eps = list(range(1,self._total_episodes_counter)) 

        total_rew=[]

        j=0
        for i in range(1,len(self.list_reward)):
            j+=self.list_reward[i]
            total_rew.append(j)

        plt.title('MushroomRL - Beam Selection environment')
        plt.xlabel('Episodes')
        plt.ylabel('Accumulated Rewards')
        
        plt.plot(total_eps, total_rew, 'g')
...
# -----------------------------------------------------------------
    ...
    def reset(self, initial_states=None):
        ...
        self._state = self._preprocess(self.mdp.reset())

        #sum the rewards each episode 
        sum_rewards = sum(self._reward_acc) 
        
        #keeping the sum of each episode's rewards in a list
        self.list_reward.append(sum_rewards)
